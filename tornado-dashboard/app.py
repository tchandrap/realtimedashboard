...
class MainHandler(tornado.web.RequestHandler):  # Class that renders login page
    ...
    def get(self):
        detail_template = template_env.get_template(
            "detail.html")  # Loads tenplate
    ...
    def post(self):
        ...
        result = r.table(PROJECT_TABLE).insert(
            {"name": name, "email": email}, conflict="error").run(threaded_conn)
        print 'log: %s inserted successfully' % result
        self.write(home_template.render({"name": email}))
...
if __name__ == "__main__":
    ...
    tornado_app = tornado.web.Application([
        ...
        (r'/static/(.*)', tornado.web.StaticFileHandler,
         {'path': static_folder})  # Define static folder
    ])
